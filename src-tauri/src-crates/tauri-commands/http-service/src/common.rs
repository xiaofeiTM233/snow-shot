...
/// 日志截断，避免图片 base64 / 大响应刷爆日志
pub(crate) fn log_truncate(value: &str, max_chars: usize) -> String {
...
/// 发送 POST 请求并记录请求/响应 info 日志，返回状态码与响应文本
/// `request_body_for_log` 用于日志展示，图片等大体积内容由调用方先做摘要
pub(crate) async fn post_and_log(
    client: &reqwest::Client,
    log_tag: &str,
    url: &str,
    request_body_for_log: &str,
    ...
    log::info!(
        ...
        log_tag,
        url,
        log_truncate(request_body_for_log, 2000)
    );

    let response = build_request(client.post(url))
        .send()
        .await
        .map_err(|e| format!("{} request failed: {}", log_tag, e))?;
    let status = response.status();
    let body = response
        .text()
        .await
        .map_err(|e| format!("{} read response failed: {}", log_tag, e))?;

    log::info!(
        "{} response, status: {}, body: {}",
        log_tag,
        status,
        log_truncate(&body, 2000)
    );
