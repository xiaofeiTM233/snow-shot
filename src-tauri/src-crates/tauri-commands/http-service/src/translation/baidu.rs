...
use crate::common::{build_http_client, post_and_log};
...
async fn get_access_token(api_key: &str, secret_key: &str) -> Result<String, String> {
    ...
    log::info!("[baidu_translation] token request");
    let body: BaiduTokenResponse = client
        ...
        .send()
        .await
        .map_err(|e| format!("[baidu_translation] Baidu token request failed: {}", e))?
        .json()
        .await
        .map_err(|e| format!("[baidu_translation] Baidu token parse failed: {}", e))?;
    log::info!(
        "[baidu_translation] token response, has_token: {}",
...
pub(crate) async fn translate_image_as_ocr(
    ...
    let request_body_for_log = format!(
        "(multipart, from: {}, to: {}, image: {} bytes)",
        source,
        target,
        image_bytes.len()
    );
    ...
    let form = reqwest::multipart::Form::new()
        ...
        .text("to", target.clone())
        .text("v", "3");

    let client = build_http_client()?;
    let (_status, body) = post_and_log(
        &client,
        "[baidu_translation:image]",
