...
use crate::common::{
    build_http_client, hmac_sha256, post_and_log, sha256_hex, utc_datetime_from_unix,
};
...
async fn post_mt_api(
    ...
    let now = SystemTime::now()
        ...
        .map_err(|e| format!("[aliyun_mt] Failed to get current time: {}", e))?;
    ...
    let signed_headers = "content-type;host;x-acs-action;x-acs-content-sha256;x-acs-date;x-acs-signature-nonce;x-acs-version";
    let canonical_headers = format!(
        "content-type:application/x-www-form-urlencoded; charset=utf-8\nhost:{}\nx-acs-action:{}\nx-acs-content-sha256:{}\nx-acs-date:{}\nx-acs-signature-nonce:{}\nx-acs-version:{}\n",
        ALIYUN_MT_HOST, action, payload_hash, x_acs_date, nonce, ALIYUN_MT_VERSION
    );
    ...
    let log_tag = format!("[aliyun_mt:{action}]");
    // 服务端偶发 503（ServiceUnavailable，官方建议重试），最多重试 2 次
    let mut attempt = 0;
    let (status, body) = loop {
        let (status, body) = post_and_log(
            ...
            |request| {
                request
                    ...
                    .header("X-ACS-Signature-Nonce", nonce.clone())
        ...
        if status.as_u16() == 503 && attempt < 2 {
            attempt += 1;
            log::warn!("[aliyun_mt] service unavailable, retry {attempt}/2");
...
pub(crate) async fn translate_image_as_ocr(
    ...
    // 成功时 Code 为 200（数字或字符串），其余为错误码
    if let Some(code) = &response.code {
        if !code.is_null() && code.as_i64() != Some(200) && code.as_str() != Some("200") {
            return Err(format!(
