...
pub async fn translate_text(
    ...
    log::info!(
        "[translate_text_online] request, service: {}, from: {}, to: {}, segments: {}, chars: {}",
        config.service_type,
        from,
        to,
        texts.len(),
    ...
    let result = if config.service_type == YOUDAO_TEXT_SERVICE_TYPE
    ...
    match &result {
        Ok(result) => log::info!(
            "[translate_text_online] result, service: {}, segments: {}, chars: {}",
            config.service_type,
            result.results.len(),
            result
                .results
        ...
        Err(error) => log::warn!("[translate_text_online] failed: {}", error),
    }
