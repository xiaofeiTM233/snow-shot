...
use crate::common::{build_http_client, normalize_error_code, post_and_log};
...
/// 将表单参数拼接为请求日志，图片 base64（q）仅记录长度
fn form_body_for_log(params: &[(&str, String)]) -> String {
    params
        .iter()
        .map(|(key, value)| {
            if *key == "q" && value.chars().count() > 200 {
                format!("{key}=<base64, {} chars>", value.chars().count())
            } else {
                format!("{key}={value}")
            }
        })
...
async fn translate_text_once(
    ...
    let (_status, body) = post_and_log(
        ...
        YOUDAO_TEXT_ENDPOINT,
        &form_body_for_log(&params),
...
async fn translate_text_batch(
    ...
    let (_status, body) = post_and_log(
        &client,
        "[youdao_translation:batch]",
        YOUDAO_BATCH_TEXT_ENDPOINT,
        &form_body_for_log(&params),
        |request| request.form(&params),
    )
    .await?;
...
async fn translate_text_llm_once(
    ...
    let (_status, body) = post_and_log(
        &client,
        "[youdao_translation:llm]",
        YOUDAO_LLM_TEXT_ENDPOINT,
        &form_body_for_log(&params),
        |request| request.form(&params),
    )
    .await?;
...
pub(crate) async fn translate_image_as_ocr(
    ...
    let (_status, body) = post_and_log(
        &client,
        "[youdao_translation:image]",
        YOUDAO_IMAGE_TRANSLATION_ENDPOINT,
        &form_body_for_log(&params),
        |request| request.form(&params),
    )
    .await?;
